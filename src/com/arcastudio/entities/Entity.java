...
import com.arcastudio.world.Node;
...
public class Entity {
	...
	// Método para organização das camadas
	...
	public void followPath(List<Node> path) {
		if (path != null) {
			if (path.size() > 0) {
				...
				// xprev = x;
				// yprev = y;
				if (x < target.x * 16 && !isCollidding(this.getX() + 1, this.getY())) {
					x++;
				} else if (x > target.x * 16 && !isCollidding(this.getX() - 1, this.getY())) {
					x--;
				}

				if (y < target.y * 16 && !isCollidding(this.getX(), this.getY() + 1)) {
					y++;
				} else if (y > target.y * 16 && !isCollidding(this.getX(), this.getY() - 1)) {
					y--;
				}
